...
.dashboardToday{grid-column:1/-1}
.todayItem{display:grid;grid-template-columns:72px 1fr auto;gap:10px;align-items:center;padding:9px 0;border-top:1px solid #304237}
.todayItem:first-child{border-top:0}
.todayTime{font-size:1.08rem;font-weight:850}
.todayDone{opacity:.55}
.todayNext{background:#2a3c30;border-radius:9px;padding:9px}
.todayEmpty{color:#a8b7ad;padding-top:7px}
@media(max-width:760px){.dashboardGrid{grid-template-columns:1fr}.todayItem{grid-template-columns:64px 1fr}}
...
<section class="card dashboardHero">
  ...
  <div class="dashboardGrid">
    ...
    <div class="dashboardBlock dashboardToday">
      <div class="top">
        <div>
          <div class="dashboardLabel">Heute geplant</div>
          <div id="dashTodaySummary" class="dashboardSub">Termine werden geladen …</div>
        </div>
        <button class="secondary" onclick="document.getElementById('upcomingPrograms').scrollIntoView({behavior:'smooth'})">Zeitplan öffnen</button>
      </div>
      <div id="dashTodayList" style="margin-top:8px"></div>
    </div>
  </div>
...
function renderTodayDashboard(){
    const target=document.getElementById('dashTodayList');
    const summary=document.getElementById('dashTodaySummary');
    if(!target||!summary)return;

    const now=parseControllerNow();
    const today=allOccurrences(now,1)
        .filter(x=>sameCalendarDay(x.date,now))
        .sort((a,b)=>a.date-b.date);

    if(!today.length){
        summary.textContent='Keine Bewässerung für heute geplant';
        target.innerHTML='<div class="todayEmpty">Heute stehen keine aktiven Programme an.</div>';
        return;
    }

    const future=today.filter(x=>x.date>=now);
    const next=future.length?future[0]:null;
    const doneCount=today.length-future.length;
    summary.textContent=`${today.length} Termin${today.length===1?'':'e'} · ${doneCount} vergangen · ${future.length} noch offen`;

    target.innerHTML=today.map(x=>{
        const done=x.date<now;
        const isNext=next&&x.program.index===next.program.index&&x.date.getTime()===next.date.getTime();
        const state=isNext?'NÄCHSTER':(done?'ERLEDIGT':'GEPLANT');
        const cls=isNext?'todayItem todayNext':(done?'todayItem todayDone':'todayItem');
        const badgeClass=isNext?'warn':(done?'off':'ok');
        return `<div class="${cls}"><div class="todayTime">${formatTime(x.date)}</div><div><b>Programm ${x.program.id}</b> · Ventil ${x.program.valve+1}<div class="dashboardSub">${x.program.durationMinutes} Minuten</div></div><div><span class="badge ${badgeClass}">${state}</span> <button class="secondary" onclick="editProgram(${x.program.index})">Öffnen</button></div></div>`;
    }).join('');
}
